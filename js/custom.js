...
window.onload = function(){
	/*
	 function loadJSON(callback) {   
		...
		xobj.open('GET', 'response.json', true); // Replace 'my_data' with the path to your file
	...
	var transData = json.deals;
	var fromEle = document.getElementById("fromData");
	var travelFrom = function(transData, prop) {
		var uniqArrFrom = []; 
		var htmldatafrom ='';
		for(var i =0; i< transData.length; i++){
			var from = transData[i].departure;
			if(uniqArrFrom.indexOf(from)  == -1 ){
				uniqArrFrom.push(transData[i].departure);
				 htmldatafrom = "<option value="+transData[i].departure+">"+transData[i].departure+"</option>";
				 prop.innerHTML += htmldatafrom;
			}	
		}
	}
	var fromTo = document.getElementById("fromTo");
		var travelTo = function(transData, prop) {
		var uniqArrTo = []; 
		var htmldatato ='';
		for(var i =0; i< transData.length; i++){
			var to = transData[i].arrival;
			if(uniqArrTo.indexOf(to)  == -1 ){
				uniqArrTo.push(transData[i].arrival);
				htmldatato = "<option value="+transData[i].arrival+">"+transData[i].arrival+"</option>";
				prop.innerHTML += htmldatato;
			}	
		}
	}
	travelFrom(transData, fromEle);
	travelTo(transData, fromTo); 
}
function searchTravelPlan(transData){ 
	$('#resultData').html('');
	var frC = $('#fromData').val();
	var toC = $('#fromTo').val(); 
	if(frC == "from"){
		$('#fromData').css({'border':'red solid 1px'});
		return false;
	}
	$('#fromData').css({'border':''});
	if(toC == "to"){
		$('#fromTo').css({'border':'red solid 1px'});
		return false;
	}
	$('#fromTo').css({'border':''});
	var htmldata = getData(frC, toC);  
	$('#resultData').html(htmldata); 
	return; 
} 
function getData(from, to) {
	...
	var j = 0;
	var b = json;
	var len = b.deals.length;
	for(i=0; i<len; i++) {
		if(b.deals[i].arrival == to && b.deals[i].departure == from ) {
		    city[j] = {
				from: b.deals[i].departure, 
				to:b.deals[i].arrival, 
				cost: b.deals[i].cost, 
				tran: b.deals[i].transport, 
				hours:  b.deals[i].duration.h,
				minute:  b.deals[i].duration.m
			}
			j++;
			if(j == 3) break;
		}
	}
	if($('#cheapest').is(':checked')){
	...
	for(var k =0; k< city.length; k++) {
		result +=  '<div class="dataRows"><span>'+city[k].from+'</span> > <span>'+city[k].to+'</span> <span class="dataPrice">'+city[k].cost+' Euro</span><br/><span class="dataTrans">'+city[k].tran+' ABC for '+city[k].hours+'h '+city[k].minute+'</span></div>';
	}
	return result;
}
$(document).ready(function() {
	$('input.checkOne').on('change', function() {
